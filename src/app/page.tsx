        ...
        {tier.features.map(function(f, i) {
          return (
            <div key={i} className="flex items-start gap-3">
              <CheckCircle className={'w-4 h-4 mt-0.5 shrink-0 '+(isDark?'text-emerald-400':'text-green-500')} />
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map(function(link) {
              return <a key={link.label} href={link.href} className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-emerald-600 rounded-lg hover:bg-emerald-50 transition-all">{link.label}</a>;
            })}
          </div>
          <div className="hidden lg:flex items-center gap-3">
            <button onClick={() => setShowLangPicker(!showLangPicker)} className="px-3 py-2.5 text-sm font-medium text-gray-700 hover:text-emerald-600 transition flex items-center gap-1 rounded-lg hover:bg-gray-50 relative">
                ...
                <div className="absolute right-0 top-full mt-2 bg-white border border-gray-200 rounded-xl shadow-xl p-2 z-50 w-52">
                  {Object.entries(langMap).map(([code, name]) => (
                    <button key={code} onClick={() => { setLanguage(code as any); setShowLangPicker(false); }}
                      className={"w-full text-left px-3 py-2 text-sm rounded-lg transition " + (language === code ? "bg-emerald-50 text-emerald-700 font-medium" : "text-gray-700 hover:bg-gray-50")}>
                      {name}
                    </button>
                  ))}
                </div>
      ...
      {/* FEATURES */}
      <section id="features" className="py-28 px-4 sm:px-6 bg-gradient-to-b from-white via-emerald-50/30 to-white">
                  ...
                  {[
                    ...
                    return (
                      <div key={i} className="flex items-start gap-4 p-4 rounded-xl bg-white/5 hover:bg-white/10 transition">
                        <div className="w-10 h-10 rounded-lg bg-emerald-500/20 flex items-center justify-center shrink-0"><item.icon className="w-5 h-5 text-emerald-400" /></div>
        ...
        <div className="max-w-7xl mx-auto">
          <Reveal>
            <div className="text-center mb-16">
              ...
              <h2 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-6">Plans That{' '}<span className="bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">Grow With You</span></h2>
      ...
      <section className="py-28 px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center text-white">
          <Reveal>
            ...
            <p className="text-xl text-emerald-100 mb-10 max-w-2xl mx-auto leading-relaxed">Start free today. No credit card required. Scale when you are ready.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/register" className="group px-10 py-4 bg-white text-emerald-600 font-bold rounded-2xl hover:bg-gray-50 transition-all shadow-2xl text-lg flex items-center justify-center gap-2 hover:scale-105">
            ...
            <div className="flex items-center justify-center gap-8 mt-10 text-sm text-emerald-200">
