...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={"fixed lg:static inset-y-0 left-0 z-50 w-64 bg-emerald-950 text-slate-300 flex flex-col transition-transform duration-300 " + (sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={"flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition " + (isActive ? "bg-emerald-600 text-white" : "hover:bg-emerald-800 hover:text-white")}>
                {(() => {
        ...
        <div className="px-3 py-4 border-t border-emerald-800">
          <div className="flex items-center gap-3 px-3 py-2">
            <div className="w-9 h-9 rounded-full bg-emerald-600 flex items-center justify-center text-white text-sm font-medium">
